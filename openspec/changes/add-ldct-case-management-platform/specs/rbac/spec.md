...
系統應（SHALL）提供「管理者」角色，得執行統計分析、**報表資料集**之存取或下載（與 `reporting`）、**Power BI** 報表之使用（依院方工作區權限），以及使用者／角色管理（若本系統含後台）。

#### Scenario: 管理者存取報表與資料集

- **WHEN** 管理者依院方流程使用報表資料集或開啟已發佈之 Power BI 報表
- **THEN** 可取得 `reporting` 所定義之分析維度對應之資料或視覺化（權限範圍內）
