...
系統應（SHALL）支援自**個案總清單**或其他院方約定之多選介面選取多筆個案，批次發送簡訊（受權限與範本規範約束；本變更不交付獨立「追蹤清單」模組）。

#### Scenario: 批次選取

- **GIVEN** 使用者於**個案總清單**（或院方約定之同等多選介面）勾選多筆未結案個案
- **WHEN** 執行批次簡訊
- **THEN** 系統對每筆發送並逐筆記錄結果（成功／失敗），且得與 **`follow-up-logging`** 對應（見該 spec「與簡訊發送紀錄之對應」）
